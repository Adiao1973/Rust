...
- [Rust学习之路](#rust学习之路)
  ...
  - [Rust智能指针](#rust智能指针)
    - [基本概念](#基本概念)
    - [Deref（提供了自动解引用的能力）](#deref提供了自动解引用的能力)
    - [Drop（析构时自动调用）](#drop析构时自动调用)
    - [Box（在堆内存中分配值）](#box在堆内存中分配值)
    - [Rc（提供多次借用的能力）](#rc提供多次借用的能力)
    - [RefCell（使内部可变性成为现实）](#refcell使内部可变性成为现实)
...
第一步产生一个String对象，值为“hello”。其中“hello”可以认为是类似于长度不确定的数据，需要在堆中储存。  
...
智能指针是Rust中一种特殊的数据结构。它与普通指针的本质区别在于普通指针是对值的借用，而智能指针通常拥有对数据的所有权。在Rust中，如果你想要在堆内存中定义一个对象，并不是像Java中那样`new`一个，也不是像C语言中那样手动`malloc`函数来分配内存空间。Rust中使用的是`Box::new`来对数据进行封箱，而`Box<T>`就是要介绍的智能指针之一。除了`Box<T>`之外，Rust标准库中提供的智能指针还有`Rc<T>`、`Ref<T>`、`RefCell<T>`等等。

### 基本概念

Rust的智能指针是一种特殊的数据结构，那么它特殊在哪呢？它与普通数据结构的区别在于智能指针实现了`Deref`和`Drop`这两个`traits`。实现`Deref`可以使智能指针能够解引用，而实现`Drop`则使智能指针具有自动析构的能力。

### Deref（提供了自动解引用的能力）

`Deref`有一个特性是强制隐式转换： **如果一个类型`T`实现了`Deref`，则该类型T的引用在应用的时候会被自动转换为类型U** 。
...
如果查看`Rc`的源码，会发现它并没有实现`chars()`方法，但上面这段代码却可以直接调用，这是因为`Rc`实现了`Deref`。
...
`DerefMut`和`Deref`类似，只不过它是返回可变引用的。
...
这里只有一个`drop`方法，实现了`Drop`的结构体，在消亡之前，都会调用`drop`方法。
...
可以看到`x`和`y`在生命周期结束时都去执行了drop方法。  
对智能指针的基本概念就先到这，接下来介绍各种智能指针都有什么特点。

### Box（在堆内存中分配值）

前面已经提到了Box在Rust中是用来在堆内存中保存数据使用的。  
它的使用方法非常简单：
...
可以看到这里只有一个`box`关键字，这个关键字是用来进行堆内存分配的，它只能在Rust源码内部使用。`box`关键字会调用Rust内部的`exchange_malloc`和`box_free`方法来管理内存。
...
在前面的学习中，我们知道Rust中一个值在同一时间只能有一个变量拥有所有权，但有时可能会需要多个变量拥有所有权，例如在图结构中，两个图可能对同一条边拥有所有权。  
对于这样的情况，Rust为我们提供了智能指针`Rc`（reference counting）来解决共享所有权的问题。每当我们通过`Rc`共享一个所有权时，引用计数就会加`1`。当引用计数为`0`时，该值才会被析构。  
`Rc`是单线程引用计数指针，不是线程安全类型。  
现在通过一个简单的例子来看一下`Rc`的应用。（示例来自[the book](https://doc.rust-lang.org/book/ch15-04-rc.html)）  
如果想要构造一个“双头”的链表，如下图所示，`3`和`4`都指向`5`。首先尝试使用`Box`实现。

![双头链表](README_files/10.jpg)
...
上述代码在编译时就会报错，因为`a`绑定给了`b`以后就无法绑定给`c`了。

![报错过程](README_files/11.jpg)
...
这时可以看到`a`的引用计数时`3`，这是因为这里计算的是节点`5`的引用计数，而`a`本身也是对`5`的一次绑定。这种通过`clone`方法共享所有权的引用称作 **强引用**。  
Rust还提供了另一种智能指针`Weak`，可以把它当作是`Rc`的另一个版本。它提供的引用属于弱引用。它共享的指针没有 **所有权** 。但他可以帮助我们有效的避免循环引用。

### RefCell（使内部可变性成为现实）

前面我们讲过变量的可变性和不可变性，主要针对变量的。按照前面所讲的，对于结构体来说，我们也只能控制它的整个实例是否可变。实例的具体某个成员是否可变我们是控制不了的。但是，在实际开发中，这样的场景也比较常见的。  
比如，有一个`User`结构体：
...
通常情况下，我们只能修改一个人的`name`或者`age`，而不能去修改用户的`id`。如果把`User`的实例设置成了可变状态，那就不能保证别人不会去修改`id`。  
为了应对这种情况，Rust提供了`Cell<T>`和`RefCell<T>`。它们本质上不属于智能指针，而是可以提供内部可变性的容器。内部可变性实际上是一种设计模式，它的内部是通过一些`unsafe`代码来实现的。
首先看一下`Cell<T>`的使用方法：
...
可以使用`Cell`的`set`/`get`方法来设置/获取内部的值。这有点像`Java`实体类中的`setter`/`getter`方法。这里有一点需要注意：`Cell<T>`中包裹的`T`必须要实现`Copy`才能够使用`get`方法，如果没有实现`Copy`，则需要使用`Cell`提供的`get_mut`方法来返回可变借用，而`set`方法在任何情况下都可以使用。由此可见`Cell`并没有违反借用规则。  
对于没有实现`Copy`的类型，使用`Cell<T>`还是比较不方便的，还好Rust提供了`RefCell<T>`。
...
从上面这段代码中，可以观察到`RefCell<T>`的`borrow_mut`和`borrow`方法对应了`Cell<T>`中的`set`和`get`方法。  
